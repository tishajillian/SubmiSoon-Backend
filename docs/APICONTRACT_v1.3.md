...
### v1.3 (2026-01-07)
**Added:**
- File download/preview URLs in file responses (`downloadUrl`, `previewUrl`)
- New endpoint: `GET /api/files/:fileId` - Download or preview uploaded files with access control

**Modified:**
- `FileInfoDto` now includes `downloadUrl` and `previewUrl` fields in assessment detail responses
- Save draft response now includes `uploadedFiles` array with download/preview URLs
- Submit assessment response now includes `uploadedFiles` array with download/preview URLs
- Affects endpoints: `GET /assessments/incomplete/:id`, `GET /assessments/:id`, `PUT /assessments/incomplete/:id`, `POST /assessments/incomplete/:id/submit`
---
## Authentication
- **Method:** Bearer Token
	    ...
	    },
	    {
		  ...
		  "answer": {
		    ...
		    "file": {
		      ...
		      "extension": "pdf",
		      "size": 245678,
		      "downloadUrl": "https://api.example.com/api/files/301?download=true",
		      "previewUrl": "https://api.example.com/api/files/301"
		    }
		  }
	    }
	    ...
  ...
  "data": {
    ...
    "uploadedFiles": [
      ...
      },
      {
        ...
        "size": 512340,
        "downloadUrl": "https://api.example.com/api/files/302?download=true",
        "previewUrl": "https://api.example.com/api/files/302"
      }
    ]
  },
  "success": true,
...
---
### 4. Files
#### 4.1 Download/Preview File
**Method:** `GET`  
**Path:** `/api/files/:fileId`  
**Description:** Download or preview an uploaded file. Students can access their own uploaded files.

**Path Parameters**
- `fileId` (integer, required): The file ID to retrieve

**Query Parameters**

| Query Name | Type    | Example            | Default | Notes                                      |
| ---------- | ------- | ------------------ | ------- | ------------------------------------------ |
| download   | boolean | `?download=true`   | false   | If true, forces download. If false, shows inline (preview) |

**Response 200 (file content)**
- Returns the actual file content with appropriate Content-Type header
- Content-Disposition: "attachment" if download=true, "inline" if false
- For PDFs and images: Browser will preview inline by default
- For documents (.doc, .docx): Browser will download by default

**Example Usage**
```
# Preview file inline (e.g., PDF in browser)
GET /api/files/301

# Force download
GET /api/files/301?download=true
```

**Response 404 (application/json) - File Not Found**
```json
{
  "success": false,
  "error": {
    "code": "FILE_NOT_FOUND",
    "message": "File with ID 301 not found"
  }
}
```
...
- Access control: Only file owner (student who uploaded it) can access
- Files are served directly from server storage
- Browser automatically handles file display/download based on Content-Disposition header
- Supported file types: .doc, .docx, .pdf, .jpg, .jpeg, .png
